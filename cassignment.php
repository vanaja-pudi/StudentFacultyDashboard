...
if(isset($_POST['stupload'])){
	...
	{	
		...
		$idnum=$_SESSION["id"];
		$tara=$con->query("select *from cilearner where ciid='$idnum'");
		while($same2u=$tara->fetch_array()){
			$section=$same2u['ciclass'];
		}
		$con->query("insert into ciassignment(idnum,section,prof,cont,file,time) values('$idnum','$section','$professor','$content','$fileDestination','$clock')");
