...
session_start();
$sid=$_SESSION['id'];
$stuClass=$con->query("select * from elearner where eid='$sid'");
while($same=$stuClass->fetch_array()){
	$name=$same['ename'];
}
?>
<html>
				...
				<br><br><br>
					...
					<a href="esubmit.php"id="tag"><i class="fa fa">Post</i></a>
					<a href="efac.html"id="tag"><i class="fa fa">Faculty</i></a>
	...
	<h2 class="nameSt"><?php $sid=$_SESSION["id"]; echo $name;?></h2>
				...
				$result=$con->query("select * from ealerts order by sno desc");
				$stuClass=$con->query("select eclass from elearner where eid='$sid'");
							...
							if($clDb==$cl||$clDb=='all')
							{
								?>
									<tr >
										...
										<td><h5 data-target="#<?php echo $rowDb['sno'];?>" data-toggle="modal" ><?php echo $rowDb["title"]; if(($timeDiff/86400)<3){echo'&nbsp;&nbsp;<button class="btn btn-primary btn-xs disabled"><h6>NEW</h6></button>';}?></h5></td>
