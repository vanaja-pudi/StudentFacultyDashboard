...
if(isset($_POST['stupload'])){
	...
	{	
		...
		$idnum=$_SESSION["id"];
		$tara=$con->query("select *from mlearner where mid='$idnum'");
		while($same2u=$tara->fetch_array()){
			$section=$same2u['mclass'];
		}
		$con->query("insert into massignment(idnum,section,prof,cont,file,time) values('$idnum','$section','$professor','$content','$fileDestination','$clock')");
