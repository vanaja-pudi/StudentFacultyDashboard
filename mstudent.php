...
session_start();
$sid=$_SESSION['id'];
$stuClass=$con->query("select * from mlearner where mid='$sid'");
...
<html>
				...
				<br><br><br>
					...
					<a href="mfac.html"id="tag"><i class="fa fa">Faculty</i></a>
	...
	<h2 class="nameSt"><?php $sid=$_SESSION["id"]; echo $name;?></h2>
				...
				$result=$con->query("select * from malerts order by sno desc");
				$stuClass=$con->query("select mclass from mlearner where mid='$sid'");
				while($xyz=$stuClass->fetch_array())
				{
					$cl=$xyz['mclass'];
				}
