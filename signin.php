...
$_SESSION["id"]=$sid;
$spd=$_POST['spd'];
$did=$con->query("select * from clearner where cid='$sid'");
...
if($did->num_rows>0)
{
	...
	<h2 class="nameSt"><?php $sid=$_SESSION["id"]; echo $name;?></h2>
				...
				$result=$con->query("select * from calerts order by sno desc");
				$stuClass=$con->query("select class from clearner where cid='$sid'");
...
else
{
	...
	echo '<script type="text/javascript"> alert("Sorry u r not registered")</script>';
	include "index.php";
}
?>